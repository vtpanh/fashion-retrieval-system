...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, with_fc=False):
        self.inplanes = 64
...
def resnet34_feature(pretrained=True):
    ...
    if pretrained:
        ...
        for k in loaded_state_dict:
            if k in state:
                state[k] = loaded_state_dict[k]
        rnet34.load_state_dict(state)
    
    return rnet34

def resnet18_feature(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        for k in loaded_state_dict:
            if k in state:
                state[k] = loaded_state_dict[k]
        rnet18.load_state_dict(state)

    return rnet18

models = {
    'resnet18': resnet18_feature,
    'resnet34': resnet34_feature,
    'resnet50': resnet50_feature
}
def get_model(name, pretrained):
    return models[name](pretrained)